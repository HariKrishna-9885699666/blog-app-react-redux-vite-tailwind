...
const AddPost: React.FC = () => {
  ...
  const onSubmit = (data: PostFormData) => {
    dispatch(addPost({ id: Date.now(), title: data.title, content: data.content }));
    reset();
  };
      ...
      <div className="mb-4 w-1/2">
        <textarea
          ...
          {...register("content", { 
            ...
            minLength: { value: 3, message: 'Content must be at least 3 characters' },
            maxLength: { value: 500, message: 'Content cannot exceed 500 characters' },
          })}
          className={`border p-2 w-full rounded ${errors.content ? 'border-red-500' : ''}`}
        ></textarea>
